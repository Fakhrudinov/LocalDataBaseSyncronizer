...
namespace LogicCore
{
	internal class CoreDeals
	{
		...
		private readonly string _name = "deals";
		...
		internal async Task<List<string>> FillAbsentDataAtDeals()
		{
			...
			///			- scip work at this target<---------
			///		prepare data - remove data older then its pointer
			List<DealModel>? eventsFromSource = await _repository.GetDealsOlderThanDate(
				_lastDatesFromTables.Pointer,
				_lastDatesFromTables.SourceData.Connection);
			if (eventsFromSource is null)
			{
				// break operation
				_logger.LogInformation($"Error! Fill tables '{_name}' is terminated - GetDealsOlderThanDate failed");
				_jobLog.Add($"Error! Fill tables '{_name}' is terminated - GetDealsOlderThanDate failed");
				return _jobLog;
			}
			...
			foreach (DateAndConnection table in _lastDatesFromTables.TargetDatesAndConnections)
			{
				...
				// prepare data - remove data older then its pointer
				List<DealModel> dataForTarget = new List<DealModel>();
				foreach (DealModel singleEvent in eventsFromSource)
				{
				...
				///	POST data
				int addToDBCount = await _repository.PostDataToTableDeals(dataForTarget, table.Connection);
